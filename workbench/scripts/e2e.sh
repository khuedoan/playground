...
verification_command='test -s /workspace/workbench-demo.txt && systemctl is-active workbench-sway.service workbench-wayvnc.service workbench-novnc.service code-server.service && pgrep -x blender >/dev/null'
...
printf 'Real MicroVM boot, Pi tool use, GUI services, code-server, and restart persistence passed.\n' |
